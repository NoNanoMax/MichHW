...
class Linear(object):
  ...
  def backward(dout, cache):
    ...
    N = x.shape[0]
    dw = torch.mm(torch.t(x.reshape(N, -1)), dout)
    db = dout.sum(dim=0)
    dx = torch.mm(dout, torch.t(w)).reshape(x.shape)
    #############################################################################
    #                              END OF YOUR CODE                             #
    #############################################################################
    return dx, dw, db
...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None):
    ...
    layer1 = Linear_ReLU()
    layer2 = Linear()
    out_1, c1 = layer1.forward(X, self.params["W1"], self.params["b1"])
    scores, c2 = layer2.forward(out_1, self.params["W2"], self.params["b2"])
    ###########################################################################
    #                            END OF YOUR CODE                             #
    ###########################################################################
    ...
    dx, grads["W2"], grads["b2"] = layer2.backward(dx, c2)
    dx, grads["W1"], grads["b1"] = layer1.backward(dx, c1)
...
class FullyConnectedNet(object):
  ...
  def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
    ...
    for hid in hidden_dims:
      self.params["W" + str(i)] = weight_scale * torch.randn(last_dim, hid).\
        to(dtype=dtype).to(device)
      self.params["b" + str(i)] = weight_scale * torch.randn(hid).\
        to(dtype=dtype).to(device)
      last_dim = hid
      i += 1
    ############################################################################
  ...
  def loss(self, X, y=None):
    ...
    for i in range(1, layers+1):
      if (i == layers):
        ...
        layer = Linear_ReLU()
      out, cash = layer.forward(out_hist[-1], self.params["W" + str(i)], 
          self.params["b" + str(i)])
      cash_hist.append(cash)
      if not self.use_dropout or i == layers:
        out_hist.append(out)
        continue
      out, cach_drop = Dropout().forward(out, self.dropout_param)
      drop_hist.append(cach_drop)
      out_hist.append(out)
    scores = out_hist[-1]
    ...
    j = layers
    for i in range(len(cash_hist) - 1, -1, -1):
      if i == len(cash_hist) - 1:
        dx, dw, db = Linear().backward(dx, cash_hist[i])
        dw += 2 * self.reg * self.params["W" + str(j)]
        grads["W" + str(j)] = dw
        grads["b" + str(j)] = db
        j -= 1
        continue
      if self.use_dropout:
        dx = Dropout().backward(dx, drop_hist[i])
      dx, dw, db = Linear_ReLU.backward(dx, cash_hist[i])
      dw += 2 * self.reg * self.params["W" + str(j)]
      grads["W" + str(j)] = dw
      grads["b" + str(j)] = db
      j -= 1
...
def create_solver_instance(data_dict, dtype, device):
  ...
  model = TwoLayerNet(input_dim=3*32*32, hidden_dim=50, num_classes=10,
    weight_scale=1e-3, reg=1e-5, dtype=dtype, device=device)
  solver = Solver(model, data_dict,
    optim_config={
      'learning_rate': 1.2 * 1e-1,
    },
    lr_decay=0.95,
...
def get_three_layer_network_params():
  ...
  ############################################################################
  weight_scale = 0.02   # Experiment with this!
...
def rmsprop(w, dw, config=None):
  ...
  next_w = w - config["learning_rate"] * dw / (torch.sqrt(
    config["cache"]
  ) + config["epsilon"])
  ###########################################################################
  #                             END OF YOUR CODE                            #
...
def adam(w, dw, config=None):
  ...
  next_w = w - config["learning_rate"] * mt / (torch.sqrt(vt) + 
      config["epsilon"])
  #############################################################################
  #                              END OF YOUR CODE                             #
  #############################################################################
...
class Dropout(object):
  ...
  def forward(x, dropout_param):
    ...
    if mode == 'train':
      ...
      ###########################################################################
      # Replace "pass" statement with your code
      mask = (torch.rand(x.shape) > p).to(torch.float32).to(x.device)
      out = x * mask
      ###########################################################################
      #                             END OF YOUR CODE                            #
      ###########################################################################
