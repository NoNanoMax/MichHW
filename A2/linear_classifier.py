...
def svm_loss_vectorized(W, X, y, reg):
  ...
  D = X.shape[1]
  C = W.shape[1]
  y_pred = torch.matmul(X, W) # (N, C) matrix
  y_pred -= y_pred[list(range(N)), y].reshape(-1, 1)
  y_pred += 1
  y_pred[list(range(N)), y] = 0
  y_pred = torch.max(torch.zeros_like(y_pred), y_pred)
  loss = torch.sum(y_pred) / N
  #############################################################################
  #                             END OF YOUR CODE                              #
  #############################################################################
  ...
  # Replace "pass" statement with your code
  y_pred[y_pred > 0] = 1
  dW += torch.sum(y_pred.reshape(N, 1, C) * X.reshape(N, D, 1), dim=0) / N # (D, C)
  y_num = torch.sum(y_pred, dim = 1)
  y_true = torch.zeros_like(y_pred)
  y_true[list(range(N)), y] = y_num
  dW -= torch.sum(y_true.reshape(N, 1, C) * X.reshape(N, D, 1), dim=0) / N
  dW += 2 * reg * W
  #############################################################################
  #                             END OF YOUR CODE                              #
  #############################################################################
...
def predict_linear_classifier(W, X):
  ...
  ###########################################################################
  # Replace "pass" statement with your code
  y_pred = torch.mm(X, W) # (N, C)
  y_pred = torch.argmax(y_pred, dim = 1) # (N, )
  ###########################################################################
  #                           END OF YOUR CODE                              #
  ###########################################################################
  return y_pred
...
def svm_get_search_params():
  ...
  ###########################################################################
  # Replace "pass" statement with your code
  learning_rates = [1e-3, 0.01, 5*1e-2, 1e-1, 0.5]
  regularization_strengths = [0.8, 5, 10, 20, 25]
  ###########################################################################
  #                           END OF YOUR CODE                              #
  ###########################################################################
...
def test_one_param_set(cls, data_dict, lr, reg, num_iters=2000):
  ...
  loss_hist = cls.train(X_train, y_train, lr, reg, num_iters,
            batch_size=200)

  y_train_pred = cls.predict(X_train)
  y_val_pred = cls.predict(X_val)
  train_acc = 100.0 * (y_train_pred == y_train).double().mean().item()
...
def softmax_loss_naive(W, X, y, reg):
  ...
  for i in range(N):
    y_true = y[i]
    y_pred = torch.mv(W.t(), X[i]) # (1, C)
    y_pred -= y_pred.max()
    log_sum = torch.sum(torch.exp(y_pred))
    loss -= (y_pred[y_true] -
         torch.log(log_sum))
    dW[:, y_true] -= (X[i] / N)
    # dW += (X[i].reshape(-1, 1) * torch.exp(y_pred) / (log_sum * N))
    for j in range(C):
      dW[:, j] += (torch.exp(y_pred[j]) * X[i] / (log_sum * N)) 
...
def softmax_loss_vectorized(W, X, y, reg):
  ...
  C = W.shape[1]
  y_pred = torch.mm(X, W)
  y_max = torch.max(y_pred, dim=1).values
  y_pred -= y_max.reshape(-1, 1)
  y_exp = torch.exp(y_pred)
  y_loss = y_pred[list(range(N)), y]
  y_loss -= torch.log(torch.sum(y_exp, dim=1))
  loss = -torch.sum(y_loss) / N + reg * torch.sum(W * W)

  y_exp = y_exp / torch.sum(y_exp, dim=1).reshape(-1, 1)
  dW1 = X.reshape(N, D, 1) * y_exp.reshape(N, 1, C) # (N, D, C)
  dW2_h = torch.zeros_like(y_pred) # (N, C)
  dW2_h[list(range(N)), y] = 1
  dW2 = dW2_h.reshape(N, 1, C) * X.reshape(N, D, 1)
  dW += torch.sum(dW1, dim = 0) / N
  dW -= torch.sum(dW2, dim = 0) / N
  dW += 2 * reg * W 
...
def softmax_get_search_params():
  ...
  ###########################################################################
  # Replace "pass" statement with your code
  learning_rates = [5.2*1e-2, 6*1e-2]
  regularization_strengths = [0.0002, 0.0003, 0.0004]
  ###########################################################################
  #                           END OF YOUR CODE                              #
  ###########################################################################
