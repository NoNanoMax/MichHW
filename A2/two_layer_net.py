...
def nn_forward_backward(params, X, y=None, reg=0.0):
    ...
    # Replace "pass" statement with your code
    N = X.shape[0]
    scores -= torch.max(scores, dim=1).values.reshape(-1, 1) # scores in (N, C)
    scores_true = scores[list(range(N)), y] # (N, )
    scores_exp = torch.sum(torch.exp(scores), dim=1) # (N, C) -> (N, )
    loss = torch.sum(torch.log(scores_exp) - scores_true) / N
    ...
    ###########################################################################
    # Replace "pass" statement with your code
    log_probs = scores - torch.log(scores_exp).reshape(-1, 1)
    probs = torch.exp(log_probs)
    probs_1 = probs.clone()
    probs_1[list(range(N)), y] -= 1 # (N, C) 
    grads['W2'] =  torch.mm(torch.t(h1), probs_1) / N + 2 * reg * W2
    grads['b2'] = torch.sum(probs_1, dim=0) / N


    #   W1   ReLU  W2   f
    # X -> h1 -> h -> s -> L
    # dL/dW1 = dh1/dW dh/dh ds/dh dh/dL

    probs_2 = probs.clone() # (N, C)
    probs_2[list(range(N)), y] -= 1 # (N, C) 
    Sh = torch.mm(probs_2, torch.t(W2)) # (N, C) (C, H) -> (N, H)
    Sh[h1 == 0] = 0
    grads['W1'] = torch.mm(torch.t(X), Sh) / N # (D, N) (N, C) -> (D, C)
    grads['W1'] += 2 * reg * W1
    grads['b1'] = torch.sum(Sh, dim = 0) / N # (N, H) -> (H,)
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
...
def nn_get_search_params():
  ...
  - learning_rate_decays: learning rate decay candidates
                              e.g. [1.0, 0.95, ...]
  """
  learning_rates = [0.1, 0.5, 1e0, 2, 5]
...
def find_best_net(data_dict, get_param_set_fn):
  ...
  lrs, hs, rs, lds = get_param_set_fn()
  for lr in lrs:
    for h in hs:
      for r in rs:
        for ld in lds:
          net = TwoLayerNet(3 * 32 * 32, h, 10, device=data_dict['X_train'].device, 
          ...
          if stats["val_acc_history"][-1] > best_val_acc:
              ...
              best_stat = stats 
          print("reg = " + str(r), end = ", ")
          print("hidden = " + str(h), end = ", ")
          print("learning rate = " + str(lr))
          print("accuracy =" + str(stats["val_acc_history"][-1]))
