...
def compute_distances_one_loop(x_train, x_test):
  ...
  # Replace "pass" statement with your code
  for i in range(num_train):
    x_train_flat = x_train[i].contiguous().view(-1)
    x_test_flat = x_test.contiguous().view(num_test, -1)
    dists[i] = torch.sqrt(torch.sum((x_train_flat - x_test_flat) ** 2, dim =1))
...
def compute_distances_no_loops(x_train, x_test):
  ...
  AB2 = A.mm(B.T)*2
  dists = ((A**2).sum(dim = 1).reshape(-1,1) - \
      AB2 + (B**2).sum(dim = 1).reshape(1,-1))**(1/2)
  ##############################################################################
  #                             END OF YOUR CODE                               #
  ##############################################################################
  return dists
...
def predict_labels(dists, y_train, k=1):
  ...
  idx = y_train[torch.t(idx)]
  y_pred, _ = torch.mode(idx, -1)
  ##############################################################################
  #                             END OF YOUR CODE                               #
  ##############################################################################
  return y_pred
...
def knn_cross_validate(x_train, y_train, num_folds=5, k_choices=None):
  ...
  ##############################################################################
  # Replace "pass" statement with your code
  idx_s = torch.arange(x_train.shape[0]).chunk(num_folds)
  x_train_folds = [x_train[idx] for idx in idx_s]
  y_train_folds= [y_train[idx] for idx in idx_s]
  ##############################################################################
  #                            END OF YOUR CODE                                #
  ##############################################################################
  ...
  for k in k_choices:
    k_to_accuracies[k] = []
    for to_drop in range(num_folds):
      to_get_data = [x_train_folds[i] for i in range(num_folds) if i != to_drop]
      to_get_labs = [y_train_folds[i] for i in range(num_folds) if i != to_drop]
      data_train = torch.cat(to_get_data)
      labs_train = torch.cat(to_get_labs)
      KNN = KnnClassifier(data_train, labs_train)
      k_to_accuracies[k].append(KNN.check_accuracy(x_train_folds[to_drop],
          y_train_folds[to_drop], k=k))
  ##############################################################################
  #                            END OF YOUR CODE                                #
  ##############################################################################
