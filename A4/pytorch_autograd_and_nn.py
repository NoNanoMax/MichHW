...
def three_layer_convnet(x, params):
  ...
  out = F.relu(F.conv2d(x, conv_w1, conv_b1, padding=2))
  out = F.relu(F.conv2d(out, conv_w2, conv_b2, padding=1))
  out = out.reshape(N, -1)
  out = F.linear(out, fc_w, fc_b)
  scores = out
  ##############################################################################
  #                                 END OF YOUR CODE                             
  ##############################################################################
...
def initialize_three_layer_conv_part2(dtype=torch.float, device='cpu'):
  ...
  conv_w2 = nn.init.kaiming_normal_(torch.empty(channel_2, channel_1, kernel_size_2, 
  ...
  fc_w = nn.init.kaiming_normal_(torch.empty(num_classes, 16 * 32 * 32, 
...
class PlainBlock(nn.Module):
  def __init__(self, Cin, Cout, downsample=False):
    ...
    self.net = nn.Sequential(OrderedDict([
      ...
      ('relu1', nn.ReLU()),
      ('conv1', nn.Conv2d(Cin, Cout, (3, 3), padding=1, stride=2 if downsample else 1)),
      ('bn2', nn.BatchNorm2d(Cout)),
      ('relu2', nn.ReLU()),
      ('conv2', nn.Conv2d(Cout, Cout, (3, 3), padding=1))
    ]))
    ############################################################################
    #                                 END OF YOUR CODE                         #
    ############################################################################
...
class ResidualBlock(nn.Module):
  def __init__(self, Cin, Cout, downsample=False):
    ...
    if Cin != Cout:
      self.shortcut = nn.Conv2d(Cin, Cout, (1, 1), stride=2 if downsample else 1)
    else:
...
class ResidualBottleneckBlock(nn.Module):
  def __init__(self, Cin, Cout, downsample=False):
    ...
    self.block = nn.Sequential(OrderedDict([
      ('bn1', nn.BatchNorm2d(Cin)),
      ('relu1', nn.ReLU()),
      ('conv1', nn.Conv2d(Cin, Cout//4, (1, 1), stride=2 if downsample else 1)),
      ('bn2', nn.BatchNorm2d(Cout//4)),
      ('relu2', nn.ReLU()),
      ('conv2', nn.Conv2d(Cout//4, Cout//4, (3, 3), padding=1)),
      ('bn3', nn.BatchNorm2d(Cout//4)),
      ('relu3', nn.ReLU()),
      ('conv4', nn.Conv2d(Cout//4, Cout, (1, 1)))
    ]))
    self.shortcut = nn.Identity()
    if Cin != Cout:
      self.shortcut = nn.Conv2d(Cin, Cout, (1, 1), stride=2 if downsample else 1)
