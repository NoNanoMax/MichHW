...
def compute_saliency_maps(X, y, model):
  ...
  pred = model(X)
  scores = pred[range(X.shape[0]), y]
  scores = torch.sum(scores)
  scores.backward() 
  saliency = X.grad.data # (N, 3, H, W)
  saliency = torch.abs(torch.max(X, dim=1).values)
  ##############################################################################
  #               END OF YOUR CODE                                             #
  ##############################################################################
  return saliency
...
def make_adversarial_attack(X, target_y, model, max_iter=100, verbose=True):
  ...
  for i in range(max_iter):
    pred = model(X_adv).reshape(-1)
    score = pred[target_y]
    print("iter {}, now predict {}, target score {}".format(
      i, torch.argmax(pred), F.softmax(pred)[target_y]
    ))
    ...
    score.backward()
    with torch.no_grad():
      X_adv = X_adv + learning_rate * X_adv.grad.data / torch.sum(X_adv.grad.data**2)
...
def class_visualization_step(img, target_y, model, **kwargs):
    ...
    # Replace "pass" statement with your code
    img.requires_grad_()
    pred = model(img).reshape(-1)
    score = pred[target_y] - l2_reg * torch.sum(img**2)
    score.backward()
    with torch.no_grad():
      img.data = img + learning_rate * img.grad.data
    img.grad.data.zero_()
    ########################################################################
    #                             END OF YOUR CODE                         #
    ########################################################################
